...
void OpenGLProfilerVisualizer::paintGL(QOpenGLWidget *target)
{
    P(OpenGLProfilerVisualizerPrivate);
    ...
    // Find mouse pos
    QPoint absoluteMousePos = InputManager::mousePosition();
    //QPoint relativeMousePos = absoluteMousePos - p.m_windowPosition;
    QPoint relativeMousePos = target->mapFromGlobal(absoluteMousePos);
    ...
    for (size_t i = 0; i < gpuResults.size(); ++i)
    {
        ...
        if (normalizedMarkerRect.contains(normalizedRelativeMousePos))
        {
            ...
            if (p.m_currToolTip != result.name())
            {
                ...
                QToolTip::showText(InputManager::mousePosition(), str);
            }
        }
        else
        {
            markerColor = QColor(255
                                 ,0
                                 ,0);
            if (p.m_currToolTip == result.name())
            {
